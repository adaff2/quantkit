...
| `RandomWalk` | Discrete random walk | `Delta S_t = mu Delta t epsilon_t, epsilon_t in {-1,+1}` |
| `OrnsteinUhlenbeck` | Mean-reverting OU process | `dS_t = theta (mu - S_t) dt + sigma dW_t` |
| `PoissonProcess` | Homogeneous Poisson process | `dN_t ~ Poisson(lambda dt)` |
| `CompoundPoissonProcess` | Compound Poisson with custom jump distribution | `dS_t = J_t dN_t` |
| `LevyFlight` | Lévy flight (Cauchy-distributed steps) | `Delta S_t = scale (Delta t)^(1/alpha) Xi_t, Xi_t ~ Cauchy(0,1)` |
| `FractionalBrownianMotion` | fBM with Hurst parameter H | `S_t = B_t^H, Cov(B_t^H,B_s^H)=0.5(t^(2H)+s^(2H)-|t-s|^(2H))` |
| `StableProcess` | α-stable process | `Delta S_t = scale (Delta t)^(1/alpha) L_t + loc Delta t` |
| `HestonModel` | Stochastic volatility (Heston) | `dS_t = sqrt(v_t) S_t dW_1, dv_t = theta(omega-v_t)dt + sigma sqrt(v_t) dW_2, corr(dW_1,dW_2)=rho` |
...
| `VarianceGamma` | Variance Gamma process | `Delta S_t = mu S_t Delta t + sigma sqrt(S_t) Delta W_{G_t}, dG_t ~ Gamma(Delta t/nu, nu)` |
| `ThreeTwoModel` | 3/2 stochastic volatility model | `dS_t = sqrt(v_t) S_t dW_1, dv_t = theta(omega-v_t)dt + sigma v_t^(3/2) dW_2` |
